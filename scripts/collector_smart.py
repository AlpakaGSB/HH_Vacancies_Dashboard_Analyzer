...
USER_AGENT = f"{config['project']['name']}/{config['project']['version']} ({os.getenv('EMAIL', 'pet-project')})"
...
def generate_combinations(queries):
    ...
    logger.info(f"Сгенерировано {len(combinations)} комбинаций для запросов")
    return combinations
...
def collect_for_group(combinations, group_name):
    ...
    logger.info(f"Начало сбора для группы: {group_name}")
    ...
    logger.info(f"Группа {group_name} завершена: {len(all_vacancies)} уникальных вакансий")
